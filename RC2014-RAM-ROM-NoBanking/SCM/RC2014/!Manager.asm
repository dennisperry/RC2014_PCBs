...
; **  Hardware:  RC2014                                               **      
; **                   16550 (ACE) driver added by Colin C MacArthur  **
; **               Front Panel Board 0xFF added by Colin C MacArthur  **
; **********************************************************************
...
kACE1:      .EQU 0x88           ;Base address of serial ACE #1
kPrtIn:     .EQU 0x00           ;General input port
kPrtOut:    .EQU 0x00           ;General output port
;kPrtIn:     .EQU 0xFF           ;Front Panel Board designed for RC2014 added by Colin C MacArthur  
;kPrtOut:    .EQU 0xFF           ;Front Panel Board designed for RC2014

; Include device modules
#INCLUDE    Hardware\RC2014\Serial6850.asm
#INCLUDE    Hardware\RC2014\SerialSIO2.asm
#INCLUDE    Hardware\RC2014\Serial16550.asm
...
;   Console device 3 = Serial device at $C0 (ACE #1) added by Colin C MacArthur
; Sets up hardware device flags:
;   Bit 0 = Serial 6850 ACIA #1 detected at $80
;   Bit 1 = Serial Z80 SIO   #1 detected at $80
;   Bit 2 = Serial 6850 ACIA #2 detected at $40
;   Bit 3 = Serial 16550 ACE #1 detected at $C0 added by Colin C MacArthur
Hardware_Initialise:
            XOR  A
...
; Look for 6850 ACIA #2
            ...
            CALL InitJumps      ;Set up serial vectors
@NoACIA2:
; Look for 16550 ACE #1                   added by Colin C MacArthur
            CALL RC2014_SerialACE1_Initialise
            JR   NZ,@NoACE1     ;Skip if 16550 not found
            LD   HL,iHwFlags    ;Get hardware flags
            SET  3,(HL)         ;Set 16550 present flag
            LD   HL,@PtrACE1    ;Pointer to vector list
            LD   B,2            ;Number of jump vectors
...
@PtrACE1:   ; Device #1 = Serial ACE #1 module  added by Colin C MacArthur
            .DW  RC2014_SerialACE1_InputChar
            .DW  RC2014_SerialACE1_OutputChar
...
Hardware_Devices:
            ...
            LD   DE,@szHw6850   ;Serial 6850 message Device #1
            ...
            CALL  NZ,OutputZString  ;Yes, so list it
            LD   DE,@szHw16550  ;Serial 16550 message   added by Colin C MacArthur
            BIT  3,(HL)         ;Serial 16550 present?
            CALL NZ,OutputZString ;Yes, so list it
            RET
...
@szHw16550: .DB  "1 = 16550 ACE #1   (@C0)",kNewLine,kNull
...
; Bit 3 = Serial 16550 ACE #1 detected
; Bit 4 to 7 = Not defined, all cleared to zero
iHwFlags:   .DB  0x000          ;Hardware flags
